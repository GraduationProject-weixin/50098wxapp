...
# 50098wxapp电子购物系统的设计与实现

![picture](https://raw.githubusercontent.com/GraduationProject-springboot/.github/main/img/wx.png)

### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1BPtKekEv3?p=101)
...
## 1.2 选题意义
背靠微信庞大用户群体的微信小程序，变成了当前解决用户访问应用程序入口方案的一种选择，用户的手机里面只要有微信，不需要安装新的软件，就可以直接访问指定目标，获取指定内容，解决了管理者的痛点，并且微信作为一个聊天工具，在微信里有支持的各种各样的生活服务，也为微信的用户提供了粘性，微信小程序不管是针对用户或者管理者都是一个双赢的存在。目前，传统电商及零售行业因为小程序的出现而获得了新生。本次课题针对用户网络购物，借助于微信小程序这样的平台，可以更加方便用户在线查看商品，购买所需商品，管理个人购买订单。
## 1.3 研究内容
...
### 3.1.1经济可行性
基于微信小程序的电子购物系统可以简化工作流程，提供信息处理功能，并可以长期保存数据，在后期的数据查询与编辑中耗时少，提升信息管理效率，其带来的收益比较可观。对于本系统开发，无论是开发需要使用的设备，还是开发工具等投入的成本很低。
### 3.1.2时间可行性
基于微信小程序的电子购物系统是作为本人毕设项目的一个作品，学院也充分预留了制作时间，并且在每个时间段，都安排了相应的任务，所以根据这个时间安排开展工作，是可以对本系统进行设计完成的。
### 3.1.3操作可行性
...
访问者之所以可以访问基于微信小程序的电子购物系统，主要是其数据库有该访问者的记录信息，这个记录是通过注册进行写入的数据。注册流程见下图。每当访问者进行基于微信小程序的电子购物系统注册时，其登记的每条记录都会在系统后台进行逐个判断。这个判断规则是提前设置的。一旦该访问者的登记信息在数据库中保存，即代表该访问者成为基于微信小程序的电子购物系统中的一份子。
...
基于微信小程序的电子购物系统设置了管理员权限，其用例图见下图。管理员管理不同状态的订单，管理商品和用户，审核订单评价。

![](/md/blog.004.png)

图3.4 管理员用例图

基于微信小程序的电子购物系统设置了用户权限，其用例图见下图。用户收藏商品，订购商品，管理个人订单，联系客服，在线充值。
...
第三个系统目标：实现用户需求的功能。根据对用户的调查得出的功能需求，完成基于微信小程序的电子购物系统的设计与实现。
...
设计的管理员的详细功能见下图，管理员管理不同状态的订单，管理商品和用户，审核订单评价。

![](/md/blog.006.png)

图4.1 管理员功能结构图

设计的用户的详细功能见下图，用户收藏商品，订购商品，管理个人订单，联系客服，在线充值。
...
### 4.3.1数据库E-R图
为了更好地保存基于微信小程序的电子购物系统产生的数据，就需要在数据库的设计阶段对E-R模型进行建立，以及完成数据库中的表结构的创建。之所以要单独完成数据库的设计，主要有以下几点原因：
